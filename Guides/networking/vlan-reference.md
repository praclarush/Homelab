...
| 11 | Services | `192.168.11.0/24` | `192.168.11.1` | Homelab management plane. Every stack except `media-gaming` binds here. | **Yes** -- dedicated physical NIC `enp171s0`, static `192.168.11.10` |
...
| 61 | NAS | `192.168.61.0/24` | `192.168.61.1` | Synology NAS. | **Yes** -- dedicated physical NIC `enp170s0`, static `192.168.61.10` |
...
The mini PC has two dedicated physical NICs, one per VLAN it's present
on, as configured in
[`config/netplan-00-installer-config.yaml`](../../Docker/config/netplan-00-installer-config.yaml).
VLAN 11 is untagged/native on `enp171s0` (`192.168.11.10`); VLAN 61 is
untagged/native on `enp170s0` (`192.168.61.10`), its own dedicated
switch port -- no 802.1Q tagging on the host at all. Both interfaces
are matched by MAC address in netplan, with `set-name` pinned back to
their current kernel names, since the two onboard NICs are the same
make/model and could otherwise swap names after a firmware/kernel
update. The mini PC reaches every other VLAN only insofar as
Ubiquiti's inter-VLAN routing and firewall rules allow it -- e.g.
Pi-hole answering DNS queries from Guest or IoT clients.
