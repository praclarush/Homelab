...
see [`Docker/config/`](../Docker/config/README.md) -- it's a reference
companion to the guides below, not a guide on its own.
...
| [infrastructure-networking-guide.md](stacks/infrastructure-networking-guide.md) | `infrastructure-networking` stack beyond NPM, Pi-hole, ntfy, and Tailscale: CrowdSec intrusion detection, and the cross-stack Watchtower auto-update policy |
| [llm-stack-guide.md](stacks/llm-stack-guide.md) | `llm` stack (Ollama + Open WebUI): deployment, model management, air-gapped operation, and the cross-stack `mem_limit`/OOM-killer rationale |

## Reference

Not how-to guides -- background and reference material linked from the guides above.
