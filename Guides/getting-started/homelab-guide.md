...
[`config/fstab`](../../Docker/config/fstab).
...
[`config/netplan-00-installer-config.yaml`](../../Docker/config/netplan-00-installer-config.yaml).
...
`log-opts` keys from
[`config/docker-daemon.json`](../../Docker/config/docker-daemon.json) instead
of overwriting the file -- see
[`config/README.md`](../../Docker/config/README.md) for details.
...
All five sidecars run on `SCHEDULE=@daily` with `BACKUP_KEEP_DAYS=7`,
`BACKUP_KEEP_WEEKS=4`, `BACKUP_KEEP_MONTHS=6` (7 daily / 4 weekly / 6
monthly rotation), reusing that service's own database credentials from
`.env`, and are pinned to `prodrigestivill/postgres-backup-local:14` to
match the Postgres 14 server version -- none are labeled for Watchtower
auto-update (see
[infrastructure-networking-guide.md](../stacks/infrastructure-networking-guide.md)
for the full auto-update policy), so a Postgres major-version bump
requires re-pinning this image deliberately alongside it. This is a
*logical* backup (a `pg_dump`, restorable on its own) that sits
alongside Backrest's *filesystem-level* snapshot of each service's
`./*/postgres` directory (`tools` stack, see
[tools-guide.md](../stacks/tools-guide.md)) -- a raw copy of live
Postgres data files isn't guaranteed restorable if it's taken
mid-write, so the logical dump is the consistent fallback of the two.

| Stack | Service | Backup destination |
|-------|---------|---------------------|
| `auth` | Authentik | `./authentik-postgres-backups` |
| `media-gaming` | Immich | `./immich-postgres-backups` |
| `tools` | WikiJS | `./wikijs-postgres-backups` |
| `tools` | Paperless-ngx | `./paperless-postgres-backups` |
| `tools` | Linkwarden | `./linkwarden-postgres-backups` |
...
Once every stack is deployed the first time, `Scripts/startup-all.sh`
and `Scripts/shutdown-all.sh` bring everything up or down again in this
...
[`Docker/stacks/dashboards-automation/homepage/config/`](../../Docker/stacks/dashboards-automation/homepage/config/),
...
[`homepage/config/DEPLOY.md`](../../Docker/stacks/dashboards-automation/homepage/config/DEPLOY.md):
