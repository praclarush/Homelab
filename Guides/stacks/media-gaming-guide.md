...
3. [Deploy](#3-deploy)
4. [Resource Limits and Backups](#4-resource-limits-and-backups)
...
## 4. Resource Limits and Backups

`immich-machine-learning` has a 3GB `mem_limit` -- a hard cap, not a
reservation. It's one of only three services across every stack capped
this way; see
[llm-stack-guide.md](llm-stack-guide.md#mem_limit-and-the-oom-killer)
for the full cross-stack memory rationale.

`immich-postgres-backup` runs a nightly `pg_dump` to
`./immich-postgres-backups` -- rotation schedule, image pin, and how
this relates to Backrest's filesystem-level snapshot are covered
cross-stack in
[getting-started/homelab-guide.md section 6.2](../getting-started/homelab-guide.md#62-postgres-backup-destinations).
To take a manual Immich database backup outside that schedule:
...
Jellyfin, Audiobookshelf, and Kavita each need a proxy host -- domain,
forward host/port, and websockets -- in the master table in
