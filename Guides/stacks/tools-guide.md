...
4. [Deploy](#4-deploy)
5. [Resource Limits and Backups](#5-resource-limits-and-backups)
6. [Configure Nginx Proxy Manager](#6-configure-nginx-proxy-manager)
7. [First-Time Service Setup](#7-first-time-service-setup)
...
NFS lines is at [`config/fstab`](../../Docker/config/fstab).
...
## 5. Resource Limits and Backups

`paperless-ngx` has a 2GB `mem_limit` -- its OCR pass on large documents
is one of the heavier consumers on the mini PC's 16GB of total RAM. It's
one of only three services across every stack capped this way; see
[llm-stack-guide.md](llm-stack-guide.md#mem_limit-and-the-oom-killer)
for the full cross-stack memory rationale.

WikiJS, Paperless-ngx, and Linkwarden each have their own dedicated
PostgreSQL, and each has a matching `*-postgres-backup` sidecar running
a nightly `pg_dump` to `./<service>-postgres-backups` -- rotation
schedule, image pin, and how this relates to Backrest's
filesystem-level snapshot (configured below) are covered cross-stack in
[getting-started/homelab-guide.md section 6.2](../getting-started/homelab-guide.md#62-postgres-backup-destinations).
...
(`http://192.168.11.10:81`) -- domain, forward host/port, and
websockets for each are in the master table in
[nginx-proxy-manager-guide.md section 5](../networking/nginx-proxy-manager-guide.md#section-5-creating-proxy-hosts):
pgAdmin, Stirling PDF, Mealie, n8n, IT Tools, Actual Budget,
Paperless-ngx, Grocy, Linkwarden, and Backrest. All use the existing
`*.home.bremmer.zone` wildcard certificate and the internal container
...
> **IT Tools and Stirling PDF have no login of their own** (see Section 7
...
- [ ] NPM proxy hosts created for every service in Section 6
