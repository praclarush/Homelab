# Dashboards-Automation Stack Guide

This guide covers the observability services in the
`dashboards-automation` stack beyond the base Homepage and Home
Assistant deployment (see
[getting-started/homelab-guide.md](../getting-started/homelab-guide.md)
for those two, plus first-time setup of Uptime Kuma and the
Grafana/Prometheus data source): Loki and Promtail, which complete the
Grafana observability stack by adding log aggregation alongside the
existing metrics.

> **Prerequisite:** Uptime Kuma, Grafana, and Prometheus must already
> be running, per the getting-started guide.
...
- [ ] Existing services (Grafana, Prometheus, Homepage, Home Assistant, Uptime Kuma) still accessible
